...
export class PlantAiApi {
  ...
  async identify(file: Express.Multer.File): Promise<Plant[]> {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}?include-related-images=${this.includeImage}&api-key=${this.apiKey}`, form, {
        headers: {
          ...form.getHeaders(),
        },
      });
    ...
    return plants;
      ...
      console.error('PlantNet API call failed:', error.response?.data || error.message);
      throw new BadRequestException(`Plant identification failed: ${error.message}`);
    }
  }
}
